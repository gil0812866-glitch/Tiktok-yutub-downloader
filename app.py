...
url = st.text_input("Tempel Link TikTok atau Spotify di sini:")
        ...
        if st.button("🎬 Video (No WM)"):
            with st.spinner("Lagi diproses..."):
                ...
                with yt_dlp.YoutubeDL({'format':'best','outtmpl':fid,'quiet':True}) as ydl:
                    ...
                    st.download_button("📥 Simpan Video", open(fid,"rb"), file_name=f"{info['title']}.mp4")
    with c2:
        if st.button("🎵 Audio MP3"):
            with st.spinner("Lagi diproses..."):
                ...
                with yt_dlp.YoutubeDL(opts) as ydl:
                    info = ydl.extract_info(url, download=True)
                    st.download_button("📥 Simpan MP3", open(f"{fid}.mp3","rb"), file_name=f"{info['title']}.mp3")
